...
class MyRobotActionServerNode(Node):
    ...
    def execute_trajectory_callback(self, goal_handle):
        ...
        self.hardware_interface.wait_for_joint_angles(target_angles, trajectory.joint_names)
...
class MyRobotHardwareInterface(Node):
    ...
    def wait_for_joint_angles(self, target_angles, joint_names, tolerance=0.15, timeout=10.0):
        ...
        while self.get_clock().now() - start_time < Duration(seconds=timeout):
            ...
            #time.sleep(0.01)  # Sleep for a short duration before checking again
    ...
    def get_joint_angles_from_robot(self):
        ...
        # Check if the service call was successful
        if future.result() is not None and future.result().angles:
            return self.reorder_angles(future.result().angles, future.result().joint_names, self.joints)
        else:
